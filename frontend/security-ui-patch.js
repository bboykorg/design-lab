/* Auth for newly protected endpoints + password-policy hints. */
(function () {
  if (window.__dlSecurityUiPatched) return;
  window.__dlSecurityUiPatched = true;

  var TOKEN_KEYS = ['dl_auth_token', 'dl_token', 'auth_token', 'authToken', 'token', 'access_token'];

  function token() {
    try {
      for (var i = 0; i < TOKEN_KEYS.length; i++) {
        var value = localStorage.getItem(TOKEN_KEYS[i]);
        if (value && value.length >= 16 && value.indexOf(' ') < 0) return value;
      }
    } catch (error) {}
    return '';
  }
  ...
  window.fetch = function (input, init) {
    ...
    if (url.indexOf('/api/audit') < 0) return previousFetch(input, init);
  ...
  document.addEventListener('click', function (event) {
    ...
    while (root && root.querySelectorAll('input[type="password"]').length < 3) root = root.parentElement;
    if (!root) return;
  ...
  if (window.MutationObserver) {
    new MutationObserver(updateHints).observe(document.documentElement, { childList: true, subtree: true });
  }
})();
