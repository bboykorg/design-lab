...
_PATCH_SCRIPTS = (
    ...
    "dl-design-skills.js",
    # Сразу за ним: один живой токен вместо пяти разных ключей. Слои ищут
    # токен в одном порядке и берут первый найденный, поэтому просроченная
    # строка в dl_auth_token давала 401 там, где dl_token был рабочим.
    ...
    "project-dedupe.js",
    # Сразу за ними: сверка, что удаление доехало до сервера. На телефоне
    # карточка исчезала из списка, но запрос DELETE до сервера не доходил,
    # и сайт возвращался после перезагрузки — он и не был удалён. Слой
    # досылает удаление и перечитывает список с сервера.
    "project-delete-verify.js",
    # Миниатюра сайта в карточке проекта: кадр получает десктопную ширину
    # и сжимается масштабом, иначе сайт внутри видит телефонную ширину и
    # занимает узкую полосу у левого края карточки.
    "project-thumb-fix.js",
    ...
    # на телефоне всплывающие окна непрозрачные. Работает по событиям, без
    # постоянного обхода страницы: прежний скан раз в 1,5 секунды заметно
    # тормозил слабые телефоны.
    "dl-mobile-fixes.js",
