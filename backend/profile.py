...
Любое изменение требует текущий пароль: его проверяет сервис пользователей
обычным входом, так что чужой украденный токен не позволяет угнать аккаунт.
"""
import re
import sqlite3
...
def _unsupported() -> HTTPException:
    return HTTPException(status_code=501, detail="Сервис пользователей пока не умеет менять логин и пароль: нужно добавить туда маршруты /v1/auth/change-password и /v1/auth/change-username.")
...
def _local_set_password(uid: str, new_password: str) -> None:
    ...
    with auth_module._lock, auth_module._db() as connection:
        connection.execute("UPDATE users SET pwhash=?, salt=? WHERE id=?", (auth_module._hash(new_password, salt), salt, uid))
        connection.execute("DELETE FROM tokens WHERE uid=?", (uid,))


@router.get("")
async def profile(user=Depends(require_user)):
    return {"id": user.get("id"), "username": user.get("username") or "", "plan": user.get("plan") or "free", "canEdit": True}


@router.post("/username")
async def change_username(body: UsernameIn, request: Request, user=Depends(require_user), authorization: str = Header(None)):
    ...
    if config.USER_DB_SERVICE_URL:
        ...
        if response is None or response.status_code in (404, 405): raise _unsupported()
        if response.status_code == 409: raise HTTPException(status_code=409, detail="Логин уже занят")
        if response.status_code >= 400: raise HTTPException(status_code=502, detail=f"Сервис пользователей ответил {response.status_code}: {response.text[:200]}")
...
async def change_password(body: PasswordIn, request: Request, user=Depends(require_user), authorization: str = Header(None)):
    guard("login", request, user)
    ...
    if config.USER_DB_SERVICE_URL:
        await _verify_password(current, body.password)
        payload = {"password": body.password, "old_password": body.password, "current_password": body.password, "new_password": body.new_password}
        response = await _try_paths(_PASSWORD_PATHS, payload, authorization)
        if response is None or response.status_code in (404, 405): raise _unsupported()
        if response.status_code in (400, 401, 403): raise HTTPException(status_code=401, detail="Текущий пароль неверный")
        if response.status_code >= 400: raise HTTPException(status_code=502, detail=f"Сервис пользователей ответил {response.status_code}: {response.text[:200]}")
        auth_module._REMOTE_CACHE.clear()
